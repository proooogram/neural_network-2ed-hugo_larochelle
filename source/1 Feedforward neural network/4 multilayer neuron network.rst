...
| {move up}
| {开始讲 左侧 f(x)=o(b(2)+w(2)T h(1)(x)}
| 最后 我们得到  我记作f(x)的  靠取隐藏层的 一个输出激活 {"f(x)=o(b(2)+w(2)T h(1)(x)":"f(x)"} 
| {move up}
| {move up}
| 这个隐藏层, 我们叫它h(1)(x), 1 也是因为我们有一个单隐藏层.
| 我们取 隐藏单元的那个向量{"f(x)=o(b(2)+w(2)T h(1)(x)":h(1)(x)} 再乘以w(2),  w(2)是在所有隐藏单元和输出间的连接向量.
| {move up}
| {move up}
| 这的w(2)i {右图 中间层和顶层之间, w(2)i} 是 在输出单元{顶层神经元}  和 第i个隐藏单元{中间层, h(x)i, 第i个隐藏神经元} 之间的连接权重.
| 我们计算那个{"f(x)=o(b(2)+w(2)T h(1)(x)":"w(2)T h(1)(x)"}, 再加上输出单元的偏执{"f(x)=o(b(2)+w(2)T h(1)(x)":"b(2)",标量,非向量}.
| 然后 对于输出激活函数{"output activation function"}, 我们也有  我会记作o的 一个激活函数{"f(x)=o(b(2)+w(2)T h(1)(x)":"o"}.
| 这{;输出层激活函数记作o, 而隐藏层激活函数记作g, 记号不一样的原因} 是因为 在神经网络中 我们经常 为 输出激活函数{"f(x)=o(b(2)+w(2)T h(1)(x)":"o"} 和 隐藏单元{;激活函数} {"h(x)=g(a(x))":g} 做不同选择.

.. image:: picture/1_04_multilayer_neural_network.pdf.page4-xxx.png
   :scale: 50%

| {复习二分类}
| 如果想做二分类, 我们只需用sigmoid激活函数作为输出激活.
| 这意思是 在输出端 我们能获得 输入属于类别1 的概率 {手写 p(y=1|x) }.
| {开始讲 多类别分类器}
| 但是如果我们有多个类别{"multi-class"}, 如果有多余两个类别, 那么我们需要做的是 有多个输出{"multiple outputs"}.
| 特别地 在输出层 我们想要 每个类别 有一个输出单元.
