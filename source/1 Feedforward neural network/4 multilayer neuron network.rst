...
但是 我们已经明白  我们可以 取 输入表示 :	 试图 以某方式 靠应用某个简单转化 {Rimg x y ; AND AND}  转化输入.
{move up}
如同应用 这个例子中 这的"与"函数{Rimg y ; AND}:	   
  {该"与"函数}转化输入
  {靠"与"函数}获得输入的 新表示     线性可分的 新表示    
如果这个表示{Rimg x y ; AND AND } 的计算 能被 人工神经元 做
那么 对于更复杂的问题 {Limg }   建议用 一个更复杂的模型 {Limg } ,
    该模型:    先计算    一些  计算新表示的 人工神经元 {Rimg x y; AND AND}  
再连接这些神经元到一个输出神经元 {Rimg XOR }  该输出神经元会做 剩下的工作, 
从而 完成 更复杂函数的计算.
