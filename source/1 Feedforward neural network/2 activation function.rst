...
| so in this case the activation function g  {.
| {指着上图公式g(a)=a中的g} 
| .}  takes its pre-activation and simply outputs it 	
| so if we were to plot _the_ _.this._ activation function you get a straight line {.
| {指着上图的斜直线}
| .} like this 
...
| so it takes this form {point to : sigm(a)=1/(1+exp(-a))} _which_
| _.so we'll._ some time noted as with acronym s-ig-m _and_ {not need "and"} for short {point to "sigm("}
| and it simply takes the pre-activation {point to "(a)"}     and it computes this formula {"1/(1+exp(-a))"} here 
| so it's 1 over 1 plus the exponential of minus the pre-activation 
| and so if we draw {"function image"} this function   what we see is {"see" ?? "is" } that it will squash {"squash"} the _neurons of the_ _.neuron's._  pre-activation   between 0 and 1
| we see {"image: y [0,1]"} that all output values are between 0 and 1 here 
| and so the bigger the pre-activation {"image: 曲线 x right"}    the more it will saturate towards 1
| and smaller the _.pre._ activation {"image: 曲线 x往左"} towards minus infinite    the more it will saturate towards 0 {"image 曲线 y 0"} instead
| so this neuron is always positive {"positive"}   because it's always strictly greater {"image y 0 上"} than 0
| it's bounded {"Bounded"} _that is the output of neuron_ _.the activation of the neuron cannot be smaller than 0    nor  greater than 1  {"image y 0往下  1往上  "} 
...
|       so this should be pre-activation here {squashed ... input between 中的input划掉改为pre-activation}          
